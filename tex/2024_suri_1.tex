...
\begin{document}
    ...
    \begin{enumerate}
        ...
        % Q3
        \item せっかくなのでここでフィッシャー情報量\index{ふぃっしゃーじょうほうりょう@フィッシャー情報量}の復習をしておこう。
        ...
        以下で考えるのは、分布 $f(X; \theta)$ から得たサンプル $X$ からパラメーター $\theta$ を推測するような状況である。
        ...
        \begin{proof}
            ...
            E [S(X; \theta)] 
                ...
                = 0.$
        \end{proof}
        そして、スコア関数の分散をフィッシャー情報量と定義する。
        いま $E [S(X; \theta)] = 0$ を示したので、 
        これはスコア関数の 2 乗の期待値と等しい。
        \begin{definition}[フィッシャー情報量]
            正則性条件の下、スコア関数の分散をフィッシャー情報量 $I (\theta)$ と定める。すなわち
        ...
        その倍率のことを曲率と呼ぶのであった。
        材料力学ではりを勉強したことがある方なら、
        微小変形においてたわみの 1 階微分がたわみ角、2 階微分が曲率になるということを習ったかもしれない。}、
        ...
        \begin{theorem}[クラメール・ラオの不等式]
            ...
            \end{equation}
        \end{theorem}
        等号を満たす $\hat{\theta}$ は有効\index{ゆうこう@有効}であるといい、 
        $\frac{1}{I (\theta)}$ をクラメール・ラオの下限\index{くらめーるらおのかげん@クラメール・ラオの下限}と呼ぶ。

        \begin{proof}
            \small
            不偏推定量 $\hat{\theta}$ とスコア関数 $S(X; \theta)$ につき
            \begin{equation}\label{eq:2024-suri-1-cauchy-schwarz}
        ...
        片側検定 $H_0: \mu = \mu_0, \ H_1: \mu = \mu_1 > \mu_0$ の検出力\index{けんしゅつりょく@検出力}がどのように書けるか考えてみよう。
        ...
        そもそも、クラメール・ラオの不等式\eqref{eq:2024-suri-1-cramer-rao} を示す際に使う
        分散と共分散の不等式 \eqref{eq:2024-suri-1-cauchy-schwarz} がコーシー・シュワルツの不等式そのものである。
        ...
        \begin{figure}[tb]
            ...
            \begin{tikzpicture}
                ...
                \node [below] at (a) {\small $f_0 (c)$};
                \node [below] at (aa) {\small $= z_\alpha$};
                \node [below] at (b) {\small $f_1 (c)$};
