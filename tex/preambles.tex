\usepackage{subfiles}
\usepackage{amsmath}
\usepackage{amsfonts}       % \mathbb{}
\usepackage{amssymb}        % \therefore
\usepackage{mathtools}      % \coloneq
\usepackage{ulem}           % \sout{}
\usepackage{fancyhdr}       % \pagestyle{fancy}
\usepackage{lscape}         % landscape 環境, 表の 90° 回転
\usepackage{amstext}        % \text
\usepackage{array}          % \newcolumntype
\usepackage{circledsteps}   % Circled
\usepackage{framed}
\usepackage{bm}

\usepackage{textgreek}    % ギリシャ文字
\usepackage{upgreek}      % ギリシャ文字

\usepackage{amsthm}
\newtheorem{theorem}{定理}
\newtheorem{definition}{定義}
\newtheorem{property}{性質}


% \usepackage{bookmark}
% \usepackage{cite}

\usepackage{makeidx}      % 索引
\makeindex
% \usepackage{imakeidx}      % 索引
% \makeindex[intoc, options=-s jssc_index.ist]

\usepackage{url}
\usepackage[style=authoryear, url=false, isbn=false]{biblatex}
\DeclareFieldFormat{series}{}   % series を消す
\addbibresource{references.bib}

\newcolumntype{L}{>{$}l<{$}}


\usepackage{tikz}   % tikz
\usepackage{pgfplots}
\pgfplotsset{compat=1.18}   % pgflplots のバージョン指定
\usepgfplotslibrary{groupplots}
\usetikzlibrary{arrows.meta, matrix}

% https://tex.stackexchange.com/questions/325211/fillbetween-doesnt-fill-entire-area-under-graph
\pgfmathdeclarefunction{gauss}{2}{%
    \pgfmathparse{1/(sqrt(2*pi*#2)) * exp(-( x - #1)^2/ (2*#2))}%
}

\usepackage{hyperref}     % \hypersetup

% 目次を subsubsection まで書く
\setcounter{tocdepth}{3}

% header, footer
\pagestyle{fancy}
\fancyhead[OL]{}            % Odd Left
\fancyhead[OR]{ \leftmark } % Odd Right

% \renewcommand{\sectionmark}[1]{\markboth{#1}{#1}}
\renewcommand{\sectionmark}[1]{\markboth{#1}{}}
\renewcommand{\subsectionmark}[1]{\markright{#1}}

% enumerate の番号の振り方を変える
\renewcommand{\labelenumi}{[\arabic{enumi}]}
\renewcommand{\labelenumii}{[\arabic{enumi}-\arabic{enumii}]}


% expl3 -------------------------------------
\ExplSyntaxOn
\tl_new:N \g_exam_year_tl
\tl_new:N \g_exam_subject_tl

% 年度 (ExamYear)
\NewDocumentCommand{\ExamYear}{m}{
  \tl_gset:Nn \g_exam_year_tl {#1}
  \section*{#1}
  \addcontentsline{toc}{section}{#1}
}
% 科目 (ExamSubject)
\NewDocumentCommand{\ExamSubject}{m}{
  \tl_gset:Nn \g_exam_subject_tl {#1}
  \subsection*{#1}
  \addcontentsline{toc}{subsection}{#1}
}
% 問 (ExamQuestion)
\NewDocumentCommand{\ExamQuestion}{m}{
  \subsubsection*{#1}
  \addcontentsline{toc}{subsubsection}{#1}
  \markboth{\g_exam_year_tl \space \g_exam_subject_tl \space #1}{}
}
% コメント (ExamComment)
\NewDocumentCommand{\ExamComment}{}{
  \subsubsection*{コメント}
}
\ExplSyntaxOff
% ------------------------------------------



% ハイパーリンク設定
\hypersetup{
  % しおりの設定
  % bookmarks = true,			% しおりを作る
  bookmarksnumbered = true,	% しおりのラベルを作る
  bookmarksopen = true,		% しおりのツリーを開く
  bookmarksopenlevel = 4,    	% しおりの深さ
  bookmarkstype = toc,
  % リンクカラーの設定
  colorlinks = false,				% カラーリンク
  anchorcolor = black,			% アンカーテキストの色指定（デフォルトはblack）
  linkcolor = red,				% 内部参照リンク用のカラー
  linkbordercolor = {1 0 0}, 		% RGBリンクを囲むボックスの色（デフォルトは1 0 0）
  citecolor = green,				% 文献参照用リンク用のカラー 
  filecolor = magenta,			% ローカルファイルリンクの色指定（デフォルトはmagenta）
  urlcolor = magenta,			% 外部参照しているurlの色（デフォルトはmagenta）
  % PDFプロファイルの設定
  pdfborder={1 0 0},			% RGBリンクを囲むボックスの色（デフォルトは1 0 0）
  pdftitle={},					% pdfのタイトル
  pdfauthor={},					% pdfの著者名
  pdfsubject={},				% pdfのサブタイトル
  pdfkeywords = {},				% pdfのキーワード
  % pdfpagemode = UseThumbs	% サムネイル表示
}
